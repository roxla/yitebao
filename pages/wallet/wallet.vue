	...
	<view id="WALLET">
		...
		<view class="content">
			<view v-show="current === 0" class="select_item">
				...
				<view>
					<button @click="goRouter('../vipcard/vipcard?type=wallet')" type="primary" class="button_color" style="background-color: #02ABA4;">申请会员卡</button>
				</view>
			</view>
			<view v-show="current === 1" class="select_item">
				<scroll-view :style="'height:'+height+'px'" scroll-y>
					<view class="wallet_box" v-for="(item,index) in vipList" :key="index">
							<view class="wallet_img">
								<image :src="item.typeImage" @click="showCardInfo(item)"></image>
							</view>
							<view class="wallet_set">
				...
				<view>
					<button type="primary" class="button_color" style="background-color: #02ABA4;">申请团洗卡</button>
				</view>
			</view>
		</view>
		<!-- 弹窗 -->
